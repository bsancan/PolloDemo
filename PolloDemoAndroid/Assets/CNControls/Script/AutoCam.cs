...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
		...
		[SerializeField] public int m_CameraMode = 3;	//modificacion para manejo de 1era persona
        ...
        protected override void FollowTarget(float deltaTime)
        {
			...
			//modificacion para manejo de 1era persona
			if (m_CameraMode == 3) {
				...
				if (CnControls.CnInputManager.GetAxis ("Horizontal_CH") > 0.01f) {
					if (degree <= 30f)
						degree += 0.5f;
				} else if (CnControls.CnInputManager.GetAxis ("Horizontal_CH") < -0.01f) {
					if (degree >= -30f)
						degree -= 0.5f;
				} 

				if (CnControls.CnInputManager.GetAxis ("Vertical_CH") > 0.01f) {
					if (degree2 >= -30f)
						degree2 -= 0.5f;
				} else if (CnControls.CnInputManager.GetAxis ("Vertical_CH") < -0.01f) {
					if (degree2 <= 30f)
						degree2 += 0.5f;
				}


				//m_Target.rotation = Quaternion.Euler(degree2, degree,0);

				//m_Target.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(degree2, degree,0), 4f * deltaTime);
				transform.rotation = m_Target.rotation;

				//m_Pivot.position = transform.position + offset;
